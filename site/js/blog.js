...
(function () {
  ...
  /* Local png/jpg assets have webp twins generated by tools/generate-webp.js.
     Remote images (Unsplash, Pexels) are already optimized and left alone. */
  function webpVariant(src) {
    return /^\/assets\/.+\.(png|jpe?g)$/i.test(src)
      ? src.replace(/\.(png|jpe?g)$/i, '.webp')
      : null;
  }
  function webpSource(src) {
    var webp = webpVariant(src);
    return webp ? '<source srcset="' + webp + '" type="image/webp">' : '';
  }
  ...
  function initHeroCarousel() {
    ...
    slides.forEach(function (p) {
      if (p && p.image) { var im = new Image(); im.src = webpVariant(p.image) || p.image; }
    });
    ...
    function applySlide(p) {
      ...
      if (img) {
        var heroWebp = webpVariant(p.image);
        img.onerror = function () { if (img.src !== p.image) img.src = p.image; };
        img.src = heroWebp || p.image;
